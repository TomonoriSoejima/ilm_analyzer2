...
export function TransformStatsView({ stats, config }: TransformStatsViewProps) {
  ...
  const toggleConfig = (transformId: string) => {
              ...
              {expandedTransforms.has(transform.id) && (
                <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  {transformConfig && (
                    <div>
                      <button
                        onClick={() => toggleConfig(transform.id)}
                        className="flex items-center justify-between w-full text-left mb-3"
                      >
                        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center">
                          {expandedConfigs.has(transform.id) ? (
                      ...
                      {expandedConfigs.has(transform.id) && (
                        <div className="space-y-4">
                          ...
                          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                            <h5 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Source and Destination</h5>
                            <div className="space-y-2">
                              <div>
                                <span className="text-sm text-gray-600 dark:text-gray-400">Source Indices:</span>
                                <div className="mt-1 space-x-2">
                                  {transformConfig.source.index.map((idx, i) => (
                                    <span key={i} className="inline-block px-2 py-1 text-xs bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded">
                                      {idx}
                                    </span>
                                  ))}
                                </div>
                              ...
                              <div>
                                <span className="text-sm text-gray-600 dark:text-gray-400">Destination Index:</span>
                          ...
                          {/* Group By */}
                          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                            <h5 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Group By</h5>
                            <div className="space-y-2">
                              {Object.entries(transformConfig.pivot.group_by).map(([key, config]) => (
                                <div key={key} className="bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-700">
                                  <h6 className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-2">{key}</h6>
                          ...
                          {/* Aggregations */}
                          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                            <h5 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Aggregations</h5>
                            <div className="space-y-2">
                              {Object.entries(transformConfig.pivot.aggregations).map(([key, config]) => (
                                <div key={key} className="bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-700">
                                  <h6 className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-2">{key}</h6>
                                  <pre className="text-xs font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                                    {JSON.stringify(config, null, 2)}
                          ...
                          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                            ...
                            <pre className="text-xs font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                              {JSON.stringify(transformConfig.settings, null, 2)}
                            </pre>
                          </div>
                          ...
                          {transformConfig.sync && (
                            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                              ...
                              <pre className="text-xs font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                                {JSON.stringify(transformConfig.sync, null, 2)}
                              </pre>
                            </div>
                          )}
                        </div>
